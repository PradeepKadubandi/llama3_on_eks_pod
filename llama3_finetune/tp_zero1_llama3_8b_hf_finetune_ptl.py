...
#         "NEURON_CC_FLAGS": "--model-type transformer --distribution-strategy=llm-training --retry_failed_compilation --cache_dir=/shared/neuron_compile_cache/",
...
def train_llama(flags):
    ...
    #tokenizer = PreTrainedTokenizerFast.from_pretrained(flags.model_id)
    ...
    # )
    strategy_cls = RayLightningNeuronXlaStrategy if flags.use_ray else NeuronXLAStrategy
    ...
    if flags.save_checkpoint:
        ...
        callbacks.append(
            ModelCheckpoint(
                ...
                monitor="global_step",
                mode="max",
                every_n_epochs=flags.checkpoint_freq,
                dirpath=flags.checkpoint_dir,
            )
        )
...
def build_args() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="/shared/Meta-Llama-3.1-8B",
